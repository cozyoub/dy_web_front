...
import en from "@/locales/en/main/MainCase";
gsap.registerPlugin(ScrollTrigger);

const KO_CASES = [
  {
    id: "case01",
    title: "디지털 트윈으로 병목공정을 발견하고 개선까지 연결하다.",
    image: "/images/main/case_img1.png",
    before: "공정별 작업시간과 병목 위치를 수작업으로 확인",
    change: "설비 데이터와 Digital Twin 관제 시스템을 연결해 병목 원인을 분석",
    after: "병목 공정을 식별하고 개선 권고와 운영조건 조정까지 지원",
    tags: ["DT", "설비관제", "생산 데이터 연계"],
  },
  {
    id: "case02",
    title: "설비 데이터를 표준화해 코일 품질을 예측하다",
    image: "/images/main/case_img2.png",
    before: "운영 파라미터와 품질 데이터가 분리되어 원인 분석이 어려움",
    change: "설비 데이터 표준화와 AI 품질 예측 모델 구축",
    after: "품질 이상을 사전에 예측하고 대응 시간을 단축",
    tags: ["AI/ML", "데이터 표준화", "품질 예측"],
  },
  {
    id: "case03",
    title: "계근대 설치부터 운송데이터 통합까지 현장 환경을 구축하다",
    image: "/images/main/case_img3.png",
    before: "현장 계근 환경과 공장별 운송 정보가 분리됨",
    change: "차량 계근대 설치, PLC 계근 시스템 구축, TMS 연동",
    after: "차량 전입, 중량, 배차와 운송 상태를 실시간 통합 운영",
...
export default function MainCase() {
  ...
  const caseItems = KO_CASES.map((item, i) => ({
    id: item.id,
    title: tr(`cases.${i}.title`, item.title),
    image: item.image,
    steps: [
      { label: tr("stepBefore", "DX 이전"), text: tr(`cases.${i}.before`, item.before) },
      { label: tr("stepChange", "전환"), text: tr(`cases.${i}.change`, item.change) },
      { label: tr("stepAfter", "DX 이후"), text: tr(`cases.${i}.after`, item.after) },
    ],
    tags: item.tags.map((tag, ti) => tr(`cases.${i}.tags.${ti}`, tag)),
  }));
          ...
          <MainSectionTitle
            ...
            title={
              <>
                ...
                <br />
                <b>{tr("titleB", "구축 결과")}</b>
                {tr("titleAfter", "로 증명합니다")}
              </>
            }
            eyebrow={<>{tr("eyebrow", "Digital Twin, AI 기반 품질 예측, 데이터 표준화 등 현장 중심의 DX·AX 기술을 적용하여 생산성과 운영 효율을 높이고,데이터 기반의 의사결정을 지원하는 실질적인 구축 성과를 만들어가고 있습니다.")}</>}
          />
        </div>
            ...
            <div key={item.id} className="main-case__card">
              ...
              <div className="main-case__step">
                ...
                <div className="main-case__tags-list">
                  <span className="main-case__step-label">{tr("solutionLabel", "적용 솔루션")}</span>
