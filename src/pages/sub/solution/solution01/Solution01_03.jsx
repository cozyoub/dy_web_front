...
import en from "@/locales/en/solution01_03";

export default function Solution01_03() {
  const tr = useTr(en);

  const faReatures = [
    tr("features.items.0", "업무 표준화를 통한 전표 처리 간소화 및 체계적인 채권·채무 관리"),
    tr(
      "features.items.1",
      "레거시 시스템, 금융 CMS, 전자세금계산서 등 타 시스템과의 연동을 통한 데이터 신뢰성 확보",
    ),
    tr("features.items.2", "업무 효율성 향상을 위한 다양한 보조 기능 및 맞춤형 커스터마이징 제공"),
    tr("features.items.3", "효율적인 자금 운영을 위한 자금 계획·예산 통제 기능 제공"),
  ];

  const faMokups = [
    {
      title: tr("mokup.items.0.title", "결재 진행 상태 실시간 확인"),
      desc: (
        <>
          {tr(
            "mokup.items.0.desc",
    ...
    {
      title: tr("mokup.items.1.title", "손익계산서의 다양한 기능 제공"),
      desc: (
        <>
          {tr(
            "mokup.items.1.desc",
    ...
    {
      title: tr("mokup.items.2.title", "CMS 연계 시스템 제공"),
      desc: (
        <>
          {tr(
            "mokup.items.2.desc",
    ...
    <>
      <div className="solution-wrap">
        <SolutionIntro
          ...
          label={tr("intro.label", "FA")}
          description={
            <>
              {tr("intro.d1", "최신 IT기술이 적용된 스마트 재무회계 시스템")}
              <br />
              {tr("intro.d2", "다양한 커스터마이징을 제공하는 기업 특화 재무회계 솔루션")}
            </>
          }
        />
        <SolutionFetures
          items={faReatures}
          title={<>{tr("features.title", "재무 데이터를 더 정확하고 스마트하게")}</>}
        />
        <SolutionFlow title={tr("flow.title", "사용자 업무 흐름도")} imgSrc="/images/sub/fa_flow.svg" />
        <div className="solution-mokup-wrapper">
          <div className="sub-inner">
            <div className="solution-title">
              <h3>{tr("mokup.title", "N·Core 재무회계는 무엇이 다를까요?")}</h3>
            </div>
